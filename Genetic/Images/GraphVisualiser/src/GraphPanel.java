...
class GraphPanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Handle the case where all nodes are at the same position
        if (minx == maxx) {
            minx -= 1;
            maxx += 1;
        }
        ...
        double scaleX = (getWidth() - 2 * edgePadding) / (maxx - minx + 1);
        double scaleY = (getHeight() - 2 * edgePadding) / (maxy - miny + 1);
    ...
    // Method to save the graph as an image
    public void saveAsImage(String filename) {
        BufferedImage image = new BufferedImage(getWidth(), getHeight(), BufferedImage.TYPE_INT_ARGB);
        Graphics2D g2d = image.createGraphics();
        paint(g2d);  // render this component to the BufferedImage
        g2d.dispose();
        try {
